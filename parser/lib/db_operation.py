...
def fetch_request(exclude, scan_type, limit_num):
    ...
    if exclude:
        ...
        sql = "SELECT rid, port, protocol, host, method, user_agent, accept, accept_language, accept_encoding, cookie, referer, post_data, path, scan_xss, scan_sqli FROM requests WHERE {} = 0 and {} order by id desc limit {}".format(scan_type, conditions_str, limit_num)
    else:
        sql = "SELECT rid, port, protocol, host, method, user_agent, accept, accept_language, accept_encoding, cookie, referer, post_data, path, scan_xss, scan_sqli FROM requests WHERE {} = 0 order by id desc limit {}".format(scan_type, limit_num)
...
def get_request_info(request):
    ...
    request_info = {}
    print 'xxxxxxxx new request info'
