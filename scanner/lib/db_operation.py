...
def fetch_request(exclude, scan_type, limit_num):
    ...
    if exclude:
        ...
        sql = "SELECT rid,port, protocol, host, method, user_agent, accept, accept_language, accept_encoding, cookie, referer, post_data, path, scan_xss, scan_sqli, content_type FROM requests WHERE {} = 0 and {} order by id desc limit {}".format(scan_type, conditions_str, limit_num)
    else:
        sql = "SELECT rid, port,protocol, host, method, user_agent, accept, accept_language, accept_encoding, cookie, referer, post_data, path, scan_xss, scan_sqli, content_type FROM requests WHERE {} = 0 order by id desc limit {}".format(scan_type, limit_num)
...
def get_request_info(request):
    ...
    request_info = {}
    #print 'lyue sun request'
    #print request
    request_info['rid'] = request[0]
    request_info['port'] = request[1]
    request_info['protocol'] = request[2]
    request_info['host'] = request[3]
    request_info['method'] = request[4]
    request_info['user_agent'] = request[5]
    request_info['accept'] = request[6]
    request_info['accept_language'] = request[7]
    request_info['accept_encoding'] = request[8]
    request_info['cookie'] = request[9]
    request_info['referer'] = request[10]
    request_info['post_data'] = request[11]
    request_info['path'] = request[12]
    request_info['scan_xss'] = request[13]
    request_info['scan_sqli'] = request[14]
    request_info['content_type'] = request[15]
